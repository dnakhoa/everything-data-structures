...
**Technical corrections.** A wrong complexity bound, an incorrect invariant, code that doesn't do what the prose says it does. These are the highest-value contributions, so please include a source (a paper, a reference implementation, or a reproducible test) so the fix can be verified rather than taken on faith.
...
Live-reloads at `http://localhost:3000`. Chapters live in `src/`, diagrams in `src/images/`, and the sidebar is generated from `src/SUMMARY.md`, so a new chapter needs an entry there or it won't appear.
